<?php ?>
<!DOCTYPE html>
<html lang="en">
	<head>
		<meta charset="UTF-8">
		<meta http-equiv="X-UA-Compatible" content="IE=edge">
		<meta name="viewport" content="width=device-width, initial-scale=1.0">
		<title>a203ea3c</title>
		<link rel="stylesheet" href="css/bootstrap.min.css">
	</head>
	<body>
		<div class="container-fluid mx-5 mt-4 px-5">
			<h2>Welcome to Autos Database</h2>
			<p class="small"><a href="login.php">Please Log In</a></p>
			<p class="small">Attempt to go to <a href="view.php">view.php</a> without logging in - it should fail with an error message.</p>
			<p class="small">Attempt to go to <a href="add.php">add.php</a> without logging in - it should fail with an error message.</p>
			<p class="small"><a href="https://www.wa4e.com/assn/autosdb/">Specification for this Application</a></p>
		</div>
		<script src="js/bootstrap.min.js"></script>
	</body>
</html>